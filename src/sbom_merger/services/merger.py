...
class SbomMerger:
    ...
    def merge_sboms(
        self,
        root_sbom_path: Path,
        dependency_sbom_paths: List[Path]
    ) -> MergeResult:
        ...
        errors, warnings = self.validator.validate_version_compatibility(
            all_docs
        )
        ...
        if errors:
            raise ValueError(
                f"Cannot merge SBOMs due to validation errors: "
                f"{'; '.join(errors)}"
            )

        merged_doc, duplicate_count = self._create_merged_document(
            root_doc, dep_docs
        )

        doc_errors, doc_warnings = self.validator.validate_document(
            merged_doc
        )
    ...
    def _create_merged_document(
        self,
        root_doc: SpdxDocument,
        dep_docs: List[SpdxDocument]
        ...
        for dep_doc in dep_docs:
            for rel in dep_doc.relationships:
                ...
                related_key = (
                    f"{dep_doc.source_file}::{rel.related_spdx_element}"
                )
        ...
        creation_info['created'] = datetime.utcnow().strftime(
            "%Y-%m-%dT%H:%M:%SZ"
        )
    ...
    def _find_main_package(self, doc: SpdxDocument) -> str:
        for rel in doc.relationships:
            if (rel.spdx_element_id == "SPDXRef-DOCUMENT" and
                    rel.relationship_type == "DESCRIBES"):
